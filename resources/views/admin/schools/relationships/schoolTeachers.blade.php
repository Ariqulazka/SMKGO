...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-schoolTeachers">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <th>
                            {{ trans('cruds.teacher.fields.profile_picture') }}
                        </th>
                        <th>
                            {{ trans('cruds.teacher.fields.school') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                        ...
                        <tr data-entry-id="{{ $teacher->id }}">
                            ...
                            <td>
                                @if($teacher->profile_picture)
                                    <a href="{{ $teacher->profile_picture->getUrl() }}" target="_blank" style="display: inline-block">
                                        <img src="{{ $teacher->profile_picture->getUrl('thumb') }}">
                                    </a>
                                @endif
                            </td>
                            <td>
                                @foreach($teacher->schools as $key => $item)
                                    <span class="badge badge-info">{{ $item->school_name }}</span>
                                @endforeach
                            </td>
                            <td>
                                @can('teacher_show')
...
@endcan
  ...
  let table = $('.datatable-schoolTeachers:not(.ajaxTable)').DataTable({ buttons: dtButtons })
